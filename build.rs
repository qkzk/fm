...
fn copy_config_to_home_dir() {
    ...
    let config_folder = tilde("~/.config");
    let copy_options = fs_extra::dir::CopyOptions::new().skip_exist(true);

    if let Err(e) = fs_extra::dir::copy(default_config_files, config_folder.as_ref(), &copy_options)
    {
        eprintln!("Couldn't copy config files to user config.\n{e:?}");
    }
}
