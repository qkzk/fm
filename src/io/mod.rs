...
//! - [`display::Display`] the displayer itself. All terminal display is made there. It's a single file, since why not ? with a single entry point. It then displays one to four windows after splitting the screen. This struct changed a lot after migration from tuikit to ratatui and is subject to a lot of internal changement. Basically, we use a single trait [`display::Canvas`] inspired by tuikit (`print_with_style` & `print` in white) which should be removed in future versions.
...
pub use args::Args;
pub use commands::*;
pub use display::{color_to_style, Canvas, Display, MIN_WIDTH_FOR_DUAL_PANE};
pub use draw_menu::*;
