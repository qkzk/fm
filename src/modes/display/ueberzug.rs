//! # copy pasta of Ueberzug-rs.
//! I didn't want to create another crate just to patch some minor changes. The original project isn't maintened anymore.
...
impl Ueberzug {
    ...
    fn run(&self, cmd: &str) -> Result<(), std::io::Error> {
        ...
        let Some(stdin) = (*ueberzug).as_mut().unwrap().stdin.as_mut() else {
